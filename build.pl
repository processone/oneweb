...
my @files;
my $topdir = getcwd;
my $dir = File::Spec->catdir($topdir, qw(chrome));
my %defs = @ARGV;
...
$defs{PREFS} = sub { extract_prefs(sub {
        $_[0] =~ /^oneweb\./ and $_[0] !~ /^oneweb\.connection\.(base|host|port|ssl|overridehost|polling|user|pass)/
